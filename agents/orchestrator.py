...
def run_finsaathi_pipeline(user_profile):
    ...
    validation_result = collect_user_data(user_profile)
    ...
    score_result = calculate_credit_score(profile)
    ...
    loan_result = recommend_loans(
        profile,
        credit_score
    )

    print(
        f"✓ {len(loan_result['recommendations'])} recommendations generated"
    )
    ...
    financial_advice = literacy_agent.explain(
        advice_prompt
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Credit Score: {result['credit_score']['score']}"
    )

    print(
        f"Risk Level: {result['credit_score']['risk_level']}"
    )
